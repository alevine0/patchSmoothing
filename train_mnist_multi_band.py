...
import utils_multi_band as utils

parser = argparse.ArgumentParser(description='PyTorch MNIST Training')
parser.add_argument('--lr', default=0.01, type=float, help='learning rate')
parser.add_argument('--bandwidth', default=2, type=int, help='width of band')
parser.add_argument('--keep', default=2, type=int, help='number of bands')
...
checkpoint_file = f'./{checkpoint_dir}/mnist_bands_lr_{args.lr}_regularization_{args.regularization}_keep_{args.keep}_bandwidth_{args.bandwidth}_epoch_{{}}.pth'
...
if args.resume:
    ...
    checkpoint_file = './{}/mnist_bands_lr_{}_regularization_{}_keep_{}_bandwidth_{}_epoch_{}_resume_{}.pth'.format(checkpoint_dir, args.lr,args.regularization, args.keep, args.bandwidth,'{}', args.resume)
...
def train(epoch):
    ...
    for batch_idx, (inputs, targets) in enumerate(trainloader):
        ...
        optimizer.zero_grad()
        outputs = net(utils.random_mask_batch_one_sample(inputs, args.keep,args.bandwidth, reuse_noise=True))
...
def test(epoch):
    ...
    for batch_idx, (inputs, targets) in enumerate(testloader):
        ...
        with torch.no_grad():
            #breakpoint()
            outputs = utils.avg_hard_forward(inputs, net, args.testsamples, args.keep,args.bandwidth)
            loss = criterion(outputs, targets)
            _, predicted = outputs.max(1)
            correct += predicted.eq(targets).sum().item()
