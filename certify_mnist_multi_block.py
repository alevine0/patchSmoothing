...
parser.add_argument('--keep', default=4, type=int, help='blocks to keep')
...
def test():
    ...
    with torch.no_grad():
        for batch_idx, (inputs, targets) in enumerate(testloader):
            ...
            predictions,  certyn = utils.predict_and_certify(inputs, net,args.keep, args.block_size,  args.size_to_certify, 10,threshold =  args.threshhold)
    ...
    print('Using block size ' + str(args.block_size) + ' with threshhold ' + str(args.threshhold) + ' and ' + str(args.keep)+' blocks')
